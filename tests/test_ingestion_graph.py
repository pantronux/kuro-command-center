...
def test_graph_endpoint_returns_dataset_chunk_and_entity_nodes(monkeypatch):
    client = _auth_client(monkeypatch)
    response = client.post(
        "/api/ingestion/upload",
        files={"file": ("Graph.md", b"# Graph\n\nPantronux links Dataset EntityAlpha and EntityBeta.")},
        data={"category": "graph", "tags": "graph", "memory_scope": "chroma_only"},
        cookies={main.COOKIE_NAME: "Bearer dummy"},
    )
    assert response.status_code == 202
    dataset_uuid = response.json()["data"]["dataset"]["dataset_uuid"]
    graph = client.get(f"/api/ingestion/graph/{dataset_uuid}", cookies={main.COOKIE_NAME: "Bearer dummy"})
    assert graph.status_code == 200
    nodes = graph.json()["data"]["nodes"]
    types = {node["type"] for node in nodes}
    assert {"dataset", "chunk", "entity"} <= types
